...
$cols = "id, company_id, unit_id, name, is_deleted";

if ($method === 'GET' && $id) {
    $stmt = $db->prepare("SELECT $cols FROM racks WHERE id = ? AND company_id = ? AND is_deleted = 0");
...
if ($method === 'GET') {
    ...
    $offset = ($page - 1) * $limit;
    $sql = "SELECT $cols FROM racks WHERE company_id = ? AND is_deleted = 0";
    $params = [$companyId];
...
if ($method === 'POST') {
    ...
    if (!$unitId) { http_response_code(400); echo json_encode(['success' => false, 'error' => 'Unit required']); exit; }
    $stmt = $db->prepare("INSERT INTO racks (company_id, unit_id, name, is_deleted) VALUES (?, ?, ?, 0)");
    $stmt->execute([$companyId, $unitId, $name]);
...
if ($method === 'PUT' && $id) {
    ...
    $name = trim($input['name'] ?? ''); if ($name === '') { http_response_code(400); echo json_encode(['success' => false, 'error' => 'Name required']); exit; }
    $unitId = !empty($input['unit_id']) ? (int)$input['unit_id'] : null;
    $stmt = $db->prepare("UPDATE racks SET name = ?, unit_id = COALESCE(?, unit_id) WHERE id = ? AND company_id = ? AND is_deleted = 0");
    $stmt->execute([$name, $unitId, $id, $companyId]);
    echo $stmt->rowCount() ? json_encode(['success' => true, 'data' => ['id' => $id]]) : json_encode(['success' => false, 'error' => 'Not found']);
    exit;
}

if ($method === 'DELETE' && $id) {
    $stmt = $db->prepare("UPDATE racks SET is_deleted = 1 WHERE id = ? AND company_id = ?");
    $stmt->execute([$id, $companyId]);
