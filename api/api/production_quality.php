...
$cols = "id, company_id, name, name_in_urdu, description, is_deleted";

if ($method === 'GET' && $id) {
    $stmt = $db->prepare("SELECT $cols FROM production_quality WHERE id = ? AND company_id = ? AND is_deleted = 0");
...
if ($method === 'GET') {
    ...
    $offset = ($page - 1) * $limit;
    $sql = "SELECT $cols FROM production_quality WHERE company_id = ? AND is_deleted = 0";
    $params = [$companyId];
...
if ($method === 'POST') {
    ...
    $stmt = $db->prepare("INSERT INTO production_quality (company_id, name, name_in_urdu, description, is_deleted) VALUES (?, ?, ?, ?, 0)");
    $stmt->execute([$companyId, $name, trim($input['name_in_urdu'] ?? ''), trim($input['description'] ?? '')]);
...
if ($method === 'PUT' && $id) {
    ...
    $name = trim($input['name'] ?? ''); if ($name === '') { http_response_code(400); echo json_encode(['success' => false, 'error' => 'Name required']); exit; }
    $stmt = $db->prepare("UPDATE production_quality SET name = ?, name_in_urdu = ?, description = ? WHERE id = ? AND company_id = ? AND is_deleted = 0");
    $stmt->execute([$name, trim($input['name_in_urdu'] ?? ''), trim($input['description'] ?? ''), $id, $companyId]);
    echo $stmt->rowCount() ? json_encode(['success' => true, 'data' => ['id' => $id]]) : json_encode(['success' => false, 'error' => 'Not found']);
    exit;
}

if ($method === 'DELETE' && $id) {
    $stmt = $db->prepare("UPDATE production_quality SET is_deleted = 1 WHERE id = ? AND company_id = ?");
    $stmt->execute([$id, $companyId]);
