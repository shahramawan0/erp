...
if ($method === 'GET' && $id) {
    $stmt = $db->prepare("SELECT $cols FROM sections WHERE id = ? AND company_id = ? AND is_deleted = 0");
...
if ($method === 'GET') {
    ...
    $offset = ($page - 1) * $limit;
    $sql = "SELECT $cols FROM sections WHERE company_id = ? AND is_deleted = 0";
    $params = [$companyId];
...
if ($method === 'POST') {
    ...
    $stmt = $db->prepare("INSERT INTO sections (company_id, name, name_in_urdu, description, is_deleted) VALUES (?, ?, ?, ?, 0)");
...
if ($method === 'PUT' && $id) {
    ...
    $name = trim($input['name'] ?? ''); if ($name === '') { http_response_code(400); echo json_encode(['success' => false, 'error' => 'Name required']); exit; }
    $stmt = $db->prepare("UPDATE sections SET name = ?, name_in_urdu = ?, description = ? WHERE id = ? AND company_id = ? AND is_deleted = 0");
...
if ($method === 'DELETE' && $id) {
    $stmt = $db->prepare("UPDATE sections SET is_deleted = 1 WHERE id = ? AND company_id = ?");
    $stmt->execute([$id, $companyId]);
