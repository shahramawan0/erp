(function() {
    ...
    if (hideSidebarToggleID) {
        ...
        // Auto-collapse sidebar on desktop by default (matches the state after clicking toggle)
        // Keeps mobile behavior unchanged (sidebar already hidden on small screens)
        window.addEventListener('load', function() {
            try {
                const isDesktop = window.matchMedia('(min-width: 80rem)').matches; // 1280px
                if (isDesktop) {
                    targetDiv1?.classList.add('active');
    ...
    if (fullscreenBtn) {
        ...
        // Enter fullscreen automatically on first load (desktop only)
        window.addEventListener('load', () => {
            try {
                const isDesktop = window.matchMedia('(min-width: 80rem)').matches;
                // Avoid auto-enter if already in fullscreen by browser/UI
                if (isDesktop && !document.fullscreenElement) {
                    // Some browsers block without user gesture; fail silently
